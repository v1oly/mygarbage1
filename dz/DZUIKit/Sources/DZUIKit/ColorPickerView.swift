...
public class ColorPickerView: UIView {
    ...
    private let onSelectColor: (UIColor) -> ()
    
    public init(onSelectColor: @escaping (UIColor) -> ()) {
    ...
    private func drawColorsButtons() {
        ...
        let boxButton = UIButton()
            ...
            boxButton.addTarget(self, action: #selector(selectColor(_:)), for: .touchUpInside)
